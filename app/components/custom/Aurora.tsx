...
const Aurora: React.FC<AuroraProps> = (props) => {
  const { colorStops = ["#40ffaa", "#4079ff"], amplitude = 1.0, className } = props;
  const propsRef = useRef<AuroraRef>(props);
  ...
  useEffect(() => {
    ...
    const colorStopsArray: number[][] = colorStops.map((hex: string) => {
